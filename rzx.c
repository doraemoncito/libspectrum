...
rzx_read_header( libspectrum_context_t *context, const libspectrum_byte **ptr,
                 const libspectrum_byte *end );
static libspectrum_error
rzx_read_creator( libspectrum_context_t *context, const libspectrum_byte **ptr,
                  const libspectrum_byte *end );
static libspectrum_error
rzx_read_snapshot( libspectrum_rzx *rzx, const libspectrum_byte **ptr,
		   const libspectrum_byte *end );
static libspectrum_error
rzx_read_input( libspectrum_rzx *rzx, const libspectrum_byte **ptr,
                const libspectrum_byte *end );
static libspectrum_error
rzx_read_frames( libspectrum_context_t *context, input_block_t *block,
...
static libspectrum_error
rzx_write_snapshot( libspectrum_context_t *context, libspectrum_byte **buffer,
...
static libspectrum_error
rzx_write_input( libspectrum_context_t *context, input_block_t *block,
                 libspectrum_byte **buffer, libspectrum_byte **ptr,
                size_t *length, int compress );
static libspectrum_error
...
static libspectrum_error
rzx_write_signed_end( libspectrum_context_t *context, libspectrum_byte **buffer,
			     ...
			     size_t count, libspectrum_byte *in_bytes )
{
  ...
  if( !input ) {
    libspectrum_print_error(
      rzx->context, LIBSPECTRUM_ERROR_INVALID,
				...
				libspectrum_snap **snap )
{
  ...
  *finished = 0;
  ...
  if( rzx->in_count != rzx->data_frame->count ) {
    libspectrum_print_error(
      rzx->context, LIBSPECTRUM_ERROR_CORRUPT,
      "libspectrum_rzx_playback_frame: wrong number of INs in frame %lu: "
        "expected %lu, got %lu",
...
libspectrum_rzx_playback( libspectrum_rzx *rzx, libspectrum_byte *byte )
{
  ...
  if( rzx->in_count >= rzx->data_frame->count ) {
    libspectrum_print_error(
      rzx->context, LIBSPECTRUM_ERROR_CORRUPT,
      "libspectrum_rzx_playback: more INs during frame %lu than stored in RZX "
        "file (%lu)",
		      ...
		      size_t length )
{
  ...
  error = libspectrum_identify_file_raw( rzx->context, &raw_type, NULL, buffer,
                                         length );
  if( error ) return error;
  ...
  if( class == LIBSPECTRUM_CLASS_COMPRESSED ) {
    ...
    error = libspectrum_uncompress_file( rzx->context, &new_buffer, &new_length,
                                         NULL, raw_type, buffer, length, NULL );
    if( error ) return error;
  ...
  while( ptr < end ) {
    ...
    default:
      libspectrum_print_error(
                  rzx->context, LIBSPECTRUM_ERROR_UNKNOWN,
                 ...
                 const libspectrum_byte *end )
{
  ...
  if( memcmp( *ptr, rzx_signature, strlen( rzx_signature ) ) ) {
    libspectrum_print_error( context, LIBSPECTRUM_ERROR_SIGNATURE,
		   ...
		   const libspectrum_byte *end )
{
  ...
  if( compressed ) {
    ...
    error = libspectrum_zlib_inflate( rzx->context, ( *ptr ) + 8,
                                      blocklength - 17, &gzsnap,
                                      &uncompressed_length );
		...
		const libspectrum_byte **ptr, const libspectrum_byte *end )
{
  ...
  if( compressed ) {
    ...
    error = libspectrum_zlib_inflate( rzx->context, *ptr, blocklength, &data,
                                      &data_length );
    if( error != LIBSPECTRUM_ERROR_NONE ) {
    ...
    *ptr += blocklength;
    ...
    error =
      rzx_read_frames( rzx->context, block, &data_ptr, data + data_length );
    ...
    libspectrum_print_error( rzx->context, LIBSPECTRUM_ERROR_UNKNOWN,
		       ...
		       libspectrum_rzx_dsa_key *key )
{
  ...
  if( key ) {
    error = rzx_write_signed_start( rzx->context, buffer, &ptr, length, key,
                                    creator );
    if( error != LIBSPECTRUM_ERROR_NONE ) return error;
  }
  ...
  for( list = rzx->blocks; list; list = list->next ) {
    ...
    case LIBSPECTRUM_RZX_SNAPSHOT_BLOCK:
      error = rzx_write_snapshot( rzx->context, buffer, &ptr, length,
    ...
    case LIBSPECTRUM_RZX_INPUT_BLOCK:
      ...
      error = rzx_write_input( rzx->context, &( block->types.input ), buffer,
                               &ptr, length, compress );
      if( error != LIBSPECTRUM_ERROR_NONE ) return error;
      break;
  ...
  if( key ) {
    error = rzx_write_signed_end( rzx->context, buffer, &ptr, length,
                                  sign_offset, key );
    if( error != LIBSPECTRUM_ERROR_NONE ) return error;
  }
...
rzx_write_signed_end( libspectrum_context_t *context, libspectrum_byte **buffer,
                      libspectrum_byte **ptr, size_t *length,
                      ptrdiff_t sign_offset, libspectrum_rzx_dsa_key *key )
{
#ifdef HAVE_GCRYPT_H
  libspectrum_error error;
  ...
  error = libspectrum_sign_data( context, &signature, &sig_length,
                                 ( *buffer + sign_offset ),
