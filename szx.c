                       ...
                       int in_flags )
{
  ...
  *out_flags = 0;
  ...
  if( libspectrum_snap_opus_active( snap ) ) {
    write_opus_chunk( buffer, block_data, snap, compress );
  }

  if( libspectrum_snap_plusd_active( snap ) ) {
    write_plsd_chunk( buffer, block_data, snap, compress );
  }
                  ...
                  libspectrum_snap *snap )
{
  ...
  write_chunk( buffer, ZXSTBID_SPECREGS, block_data );
                  ...
                  libspectrum_snap *snap, int compress )
{
  ...
  libspectrum_buffer_write( data, libspectrum_snap_spectranet_w5100( snap, 0 ),
                            0x30 );
  for( i = 0; i < 0x30; i++ ) libspectrum_buffer_write_byte( data, 0 );
...
write_chunk( libspectrum_buffer *buffer, const char *id,
             libspectrum_buffer *block_data )
{
  size_t data_length = block_data ?
    libspectrum_buffer_get_data_size( block_data ) : 0;
