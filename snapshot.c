		       ...
		       const char *filename )
{
  ...
  if( type == LIBSPECTRUM_ID_UNKNOWN ) {
    error = libspectrum_identify_file( libspectrum_snap_context( snap ), &type,
  ...
  error = libspectrum_identify_class( libspectrum_snap_context( snap ), &class,
                                      type );
  if( error ) return error;
  ...
  error = libspectrum_identify_file_raw( libspectrum_snap_context( snap ),
                                         &raw_type, filename, buffer, length );
  if( error ) return error;
  ...
  if( class == LIBSPECTRUM_CLASS_COMPRESSED ) {
    ...
    error = libspectrum_uncompress_file( libspectrum_snap_context(snap),
                                         &new_buffer, &new_length, NULL,
