...
static libspectrum_error
read_v1_block( libspectrum_context_t *context, const libspectrum_byte *buffer,
               int is_compressed, libspectrum_byte **uncompressed,
	       const libspectrum_byte **next_block,
	       const libspectrum_byte *end );
static libspectrum_error
read_v2_block( libspectrum_context_t *context, const libspectrum_byte *buffer,
               libspectrum_byte **block, size_t *length, int *page,
               const libspectrum_byte **next_block,
               const libspectrum_byte *end );
	    ...
	    int version, int compressed )
{
  ...
  if( version == 1 ) {
    ...
    error = read_v1_block( libspectrum_snap_context( snap ), buffer, compressed,
                           &uncompressed, next_block, end );
    if( error != LIBSPECTRUM_ERROR_NONE ) return error;
    ...
    error = read_v2_block( libspectrum_snap_context( snap ), buffer,
                           &uncompressed, &length, &page, next_block, end );
    if( error != LIBSPECTRUM_ERROR_NONE ) return error;
